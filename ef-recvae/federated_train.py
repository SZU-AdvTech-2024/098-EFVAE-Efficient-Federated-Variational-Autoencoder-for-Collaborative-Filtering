...
from model.ef_recvae import EFRecVAE
...
class Clients:
    def __init__(self, args, dataset):
        ...
        self.model = EFRecVAE(args, dataset)
    ...
    def train(self, uids: torch.Tensor, model_param_state_dict: dict, sampler: SamplerBase, dropout_prob: float) -> dict:
        ...
        all_sampling_items = []
        self.model.construct_prior()
        # total_loss = 0
        for uid in uids:
            pos_items = self.clients_data[uid.item()].view(1, -1).to(self.device)
            loss, sampling_items = self.model.calculate_loss(pos_items, sampler, dropout_prob)
        ...
        self.model.remove_prior_encoder()
...
class Server:
    def __init__(self, args, dataset, clients: Clients):
        ...
        self.optimizer_decoder = torch.optim.Adam([self.model.dec_embeddings.weight, self.model.dec_bias.weight], lr=args.lr, weight_decay=args.weight_decay)
    ...
    def aggregate_gradients(self, clients_grads):
        ...
        for uid, grads_dict in clients_grads.items():
            for name, grad in grads_dict.items():
                # if grad is None:
                #     aggregated_gradients[name] = None
                # else:
    ...
    def run(self, opts, n_epochs, dropout_prob, to_restore=False):
        ...
        for _ in range(n_epochs):
            for uids in uid_seq:
                ...
                clients_grads = self.clients.train(uids, self.model.state_dict(), self.sampler, dropout_prob)
