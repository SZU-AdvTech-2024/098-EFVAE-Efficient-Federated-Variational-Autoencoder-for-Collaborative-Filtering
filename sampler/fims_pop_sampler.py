...
class FIMSPopSampler(SamplerBase):
    """
        FIMS-Pop Sampler.
    """
    ...
    @torch.no_grad()
    def forward(self, query, pos_items=None, temp=1.):
    ...
    def update(self, item_embs, bias=None):
        ...
        pop_prob = self.pop_prob
        
        if bias is not None:
            pop_prob = torch.log(pop_prob) + bias.view(-1)
        
        self.bias = pop_prob
        # bias = torch.exp(pop_prob)
        # construct cb2cates
        i2pop_prob = torch.exp(self.bias)  # I
        # Expand dimensions of i2cb to match i2cates
        # i2cd01 = self.i2cd01.repeat(1, T)

        # Initialize cd2weights tensor
        self.cd2weights = torch.zeros(self.num_code, device=i2pop_prob.device, dtype=i2pop_prob.dtype)  # (C0 * C1)

        # Use scatter_add_ to add the values from i2cates to the correct positions in cb2weights
        self.cd2weights.scatter_add_(0, self.i2cd01, i2pop_prob)
        
        # Normalization
        self.cumBias = pop_prob[self.indices].view(-1)  # I
    ...
    def get_communication_cost(self):
        return sum(get_size(m) for m in (self.cb0, self.cb1, self.cd2weights, self.indices, self.indptr, self.bias, self.cumBias)) / (1 << 20)