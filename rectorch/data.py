...
class DataProcessing:
    ...
    def process(self):
        ...
        * ``unique_iid.txt`` : (`txt` file) with the item id mapping. Line numbers represent the\
        ...
        raw_data = raw_data.drop_duplicates(raw_data.columns.values[:2], keep='first')
        # print(raw_data.head())
        ...
        # print(f"length of tr_users, vd_users, and te_users: {len(tr_users)}, {len(vd_users)}, {len(te_users)}")
        ...
        # print(f"user length from raw_data: {len(pd.unique(val_data[uhead]))}")
        val_data = val_data.loc[val_data[ihead].isin(unique_iid)]
        # print(f"user length filter by unique_iid: {len(pd.unique(val_data[uhead]))}")
        test_data = raw_data.loc[raw_data[uhead].isin(te_users)]
        test_data = test_data.loc[test_data[ihead].isin(unique_iid)]

        vcnt = val_data[[uhead]].groupby(uhead).size()
        tcnt = test_data[[uhead]].groupby(uhead).size()
        # print(vcnt.index[vcnt >= 2])
        val_data = val_data[val_data[uhead].isin(vcnt.index[vcnt >= 2])]
        # print(f"val_data length: {len(vcnt)}")
        test_data = test_data[test_data[uhead].isin(tcnt.index[tcnt >= 2])]
        # print(f"test_data length: {len(tcnt)}")
    ...
    def _filter(self, data, min_u=5, min_i=0):
        def get_count(data, idx):
            return data[[idx]].groupby(idx).size()

        [uhead, ihead] = data.columns.values[:2]
        if min_i > 0:
            icnt = get_count(data, ihead)
            data = data[data[ihead].isin(icnt[icnt >= min_i].index)]

        if min_u > 0:
            ucnt = get_count(data, uhead)
            data = data[data[uhead].isin(ucnt[ucnt >= min_u].index)]
...
class DatasetManager():
    ...
    def __repr__(self) -> str:
        ...
        n_users_train = self.training_set[0].shape[0]
        n_users_val = self.validation_set[0].shape[0]
        n_users_test = self.test_set[0].shape[0]
        n_users = n_users_train + n_users_val + n_users_test
        size_dataset = self.training_set[0].nnz + self.validation_set[0].nnz + self.validation_set[1].nnz + \
            self.test_set[0].nnz + self.test_set[1].nnz
        return f"[Dataset Info]\nn_users: {n_users}\nn_item: {n_items}\nn_users_train: {n_users_train}\nn_users_val: {n_users_val}\nn_users_test: {n_users_test}\nsize_total_dataset: {size_dataset}"
